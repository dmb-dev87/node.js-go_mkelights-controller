...
$date_modifier = 0;
//$date_modifier = 19*60*60;

$server_name = "localhost";
$user_name = "control_tree";
$user_pass = "snowman11";
$db_name = "control_show_schedule_test";
...
?>	
	...
	function enable_bt()
	{
		...
		$('#snow_on').attr("href", "#");
		$('#snow_off').attr("href", "#");
		$('#soldier_on').attr("href", "#");
		$('#soldier_off').attr("href", "#");	
...
function log_to_file (log_data){
		var seed = new Date().getTime().toString();
		log_data = encodeURIComponent(log_data);
	$.ajax({
    		type: "GET",
    		url: "log-countdown-events.php",
    		data: "data="+log_data+"&seed="+seed,
    		success: function(msg){
    			 //alert(msg );
    			 if(msg.indexOf("CMD-OK") > -1)
                 {
